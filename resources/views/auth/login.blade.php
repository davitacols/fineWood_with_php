<x-guest-layout>
  <x-authentication-card class="bg-white shadow-md rounded-md">
    <x-slot name="logo">
    ...
    <form method="POST" action="{{ route('login') }}" class="mt-4">
      ...
      <div class="mt-4">
        ...
        <x-input id="email" class="block mt-1 w-full rounded-md" type="email" name="email" :value="old('email')" required autofocus autocomplete="email" />
      </div>
      ...
      <div class="flex items-center justify-between mt-4">
        ...
        <x-button class="px-4 py-2 text-white bg-indigo-500 rounded-md">
          {{ __('Log in') }}
        </x-button>
      </div>
    </form>
